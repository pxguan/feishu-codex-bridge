...
启动后终端会打印一行**带 token 的完整 URL**，用它打开即可（首跳自动换成 cookie，URL 上的 token 随即失效于地址栏）。页面含：bot 切换、概览（bridge 运行状态 + 🩺 事件订阅/后端环境诊断）、📁 项目列表（群形态 / 🔐 权限档 / 🧠 后端 / 🧵 话题数）、项目详情抽屉、📜 当日日志 SSE 实时滚动（`stream.timing` / `agent.*` 关键事件高亮）。

- **安全模型**：只绑定 `127.0.0.1`（无任何远程访问配置项）+ 每次启动随机生成的 token 鉴权 + Host/Origin 校验防 DNS rebinding。要远程管理请用飞书私聊控制台——那是带飞书身份鉴权的。
- **与飞书卡片的关系**：体验对齐「Web 能操作的飞书也能操作」——Web 的方法清单严格对齐 DM 私聊卡片（🧠 后端 / 🔐 权限 / ✋ 免@ / 🗜️ 自动压缩 / 🩺 诊断…），两面共享同一服务层（`AdminService`），不会出现两套行为。Web 只补 DM 够不着的宿主机域（日志流 / 多 bot 聚合），飞书域操作（建项目 / 建群 / 用量）仍在 DM 卡片。
- **当前为只读预览**：直读本机数据文件，不依赖 daemon 在跑；写操作（切后端、改权限等）按钮已就位，将随 daemon 进程内集成开放（届时与 DM 卡片走同一写入逻辑）。
