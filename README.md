...
机器人调用 Codex 始终是 **`approvalPolicy: "never"`**（无人工逐条审批），**沙箱就是唯一的安全闸门**。每个项目有一档**权限**，在「结果卡」或群里 `/settings` 设置卡上随时切换：
...
- **默认值**：你自己新建的项目群 = `完全访问`（与历史行为一致）；**别人把机器人拉进的存量/外部群** = `项目内只读`。**升级前没有 `mode` 字段的老项目按 `完全访问` 处理**，行为不变。
