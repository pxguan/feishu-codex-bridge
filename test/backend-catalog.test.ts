...
describe('projectCreatableBackends —— 飞书新建/绑定卡的「可选后端」过滤（创建时选定）', () => {
  ...
  it('full 档 + 全已下载 → 仍只剩 codex（claude 两后端被「用户不可见闸」hidden 隐藏；删 catalog 里的 hidden 即恢复为三条）', () => {
    // 用户不可见闸：projectCreatableBackends 恒滤掉 hidden。claude 验证完点亮后这里应回到 ['claude-acp','claude-sdk','codex-appserver']。
    expect(ids('full', () => true)).toEqual(['codex-appserver']);
  });
  ...
  it('用户不可见闸优先于「已下载」过滤：claude-sdk 即便装了也因 hidden 不显示 → 仍只剩 codex', () => {
    expect(ids('full', (e) => e.id === 'claude-sdk')).toEqual(['codex-appserver']);
  });
});

describe('用户不可见闸（hidden）—— claude 两后端对用户全隐藏，但保留注册（验证后删 hidden 点亮）', () => {
  it('claude-sdk / claude-acp 标了 hidden；codex 不 hidden', () => {
    expect(catalogById('claude-sdk')?.hidden).toBe(true);
    expect(catalogById('claude-acp')?.hidden).toBe(true);
    expect(catalogById('codex-appserver')?.hidden).toBeFalsy();
  });
  ...
  it('hidden 只挡曝光、不退注册：catalogBackendIds 仍含三条 → 与 REGISTRY 配对不破、代码仍在仓', () => {
    expect([...catalogBackendIds()].sort()).toEqual(['claude-acp', 'claude-sdk', 'codex-appserver']);
    // 工厂仍在：真要路由到 claude（如日后点亮）createBackend 仍构造得出。
    expect(createBackend('claude-sdk').id).toBe('claude-sdk');
  });

  it('任何权限档下 projectCreatableBackends 都不含 claude（picker 永不暴露）', () => {
    for (const mode of ['qa', 'write', 'full'] as const) {
      expect(projectCreatableBackends(mode, () => true).map((e) => e.id)).toEqual(['codex-appserver']);
    }
  });
});
